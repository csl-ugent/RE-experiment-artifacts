...
label_zeroes_A:
        ...
        // INSERT ALWAYS-FALSE OP HERE: Check whether one of the known keys gets the known value
        if (hashmap_get(op_map, 3916U) != (void*)5008)
            goto label_xor_B;
...
label_zeroes_B:
        // INSERT ALWAYS-TRUE OP HERE: Check whether one of the known keys gets the wrong value
        if (hashmap_get(op_map, 1000U) != (void*)1000)
        {
            if (fun + fun2 != 0)
                return false;
        }
...
label_xor_A:
            ...
            input++;
            // INSERT ALWAYS-FALSE OP HERE: Check whether one of the known keys is present
            if (!hashmap_get(op_map, 2313U))
                goto label_zeroes_A;
...
label_xor_B:
            if(i % 2 != 0) {
                // INSERT ALWAYS-TRUE OP HERE: Check whether one of the known keys is present
                if (hashmap_get(op_map, 748U))
                {
                    if (parsed_int <= 250)
                        return false;
                }
            ...
            // INSERT ALWAYS-FALSE OP HERE: Check whether parsed_int is present in op_map (choose a value that can't be)
            // When i is even, parsed_int has to be < 250, and there are no such keys in op_map
            if (!(i % 2) && hashmap_get(op_map, parsed_int))
                goto label_zeroes_B;
label_xor_C:
