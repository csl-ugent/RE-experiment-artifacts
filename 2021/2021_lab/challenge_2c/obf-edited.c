...
bool checkKeyBjorn(char* input1, char* input2)
{
    ...
    for (unsigned i = 0; i < 5; i++)
    {
        ...
        hashmap_set(op_map, 4529U, (void*)num);
        ...
        hashmap_set(op_map, 5913, (void*)7713);
...
label_joined:
    {
      ...
      long tmp = (long)hashmap_get(op_map, 191);
      tmp += strlen(input1);
      tmp *= 8;
      hashmap_set(op_map, 191, (void*)tmp);

      // if (predicate)
      if (hashmap_get(op_map, 5913) == (void*)6751)
        goto label2;
      else
        goto label1;
    }
