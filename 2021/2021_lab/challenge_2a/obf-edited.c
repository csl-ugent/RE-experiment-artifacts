...
static string_set_t* global_set_obf;
...
void setUpDataStructures() {
    ...
    // Set structure used in checking
    global_set_obf = new_set();
    add_to_set(global_set_obf, "bart");
    add_to_set(global_set_obf, "bar");
    add_to_set(global_set_obf, "apple");
    add_to_set(global_set_obf, "applet");
    add_to_set(global_set_obf, "lion");
    add_to_set(global_set_obf, "lions");
    add_to_set(global_set_obf, "sea lion");
    add_to_set(global_set_obf, "sea lions");
}
...
// SET possible solution: bart-barts-bert-bar-applets-berts-apple-bars-thomas-tiger-tigerss-rabbit
...
bool checkKeyBjorn(char* input1, char* input2)
{
    ...
    for (unsigned i = 0; i < 5; i++)
    {
        ...
        add_to_set(global_set_obf, token_obf);
        ...
        if (l >= 2) {
            token_dummy[l-1] = '\0';
            add_to_set(global_set_obf, token_dummy);
        }
    ...
    label1:
        ...
        for (int j = 0; j < 3; j++) {
            ...
            remove_from_set(global_set_obf, token_obf);
...
label_joined:
    {
      ...
      if (current_word[0] && strcmp(current_word,token_obf))
         add_to_set(global_set_obf,current_word);

      // if (predicate)
      if (!is_in_set(global_set_obf, token_obf))
        goto label2;
      else
        goto label1;
    }
